...
FESHOG::FESHOG(string instanceID) : FeatureExtractionCPU2D(instanceID) {
...
FESHOG *FESHOG::Instantiate(string instanceID) { 

	return new FESHOG(instanceID); 
}
...
void FESHOG::Setup() {
	...
	this->integral_mat = new SSFMatrix<FeatType>[nbins];

	this->ncells = this->cell_rows * this->cell_cols;
}


int FESHOG::GetNFeatures(){ //ncell*nbins
	return this->ncells*this->nbins;
}


void FESHOG::ExtractFeatures(int x0, int y0, int x1, int y1, SSFMatrix<FeatType> &m) {
...
void FESHOG::computeGradient(Mat inp, SSFMatrix<FeatType> *gradient_direction){
	bool is_verbose = false;
