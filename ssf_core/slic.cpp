...
void Slic::InitData(Mat image) {
	...
	for (int i = step; i < image.cols - step/2; i += step) {
		for (int j = step; j < image.rows - step/2; j += step) {
			...
			Point nc = FindLocalMinimum(image, Point(i,j));
...
*/
double Slic::ComputeDist(int ci, Point pixel, Vec3b colour) {
...
*
* Input : The image (Mat*) and the pixel center (CvPoint).
* Output: The local gradient minimum (CvPoint).
*/
Point Slic::FindLocalMinimum(Mat image, Point center) {
...
* Output: -
*/
void Slic::GenerateSuperpixels(Mat image, int step, int nc) {
	this->step = step;
	this->nc = nc;
	this->ns = step;
			...
			for (int k = centers[j][3] - step; k < centers[j][3] + step; k++) {
				for (int l = centers[j][4] - step; l < centers[j][4] + step; l++) {
					...
					if (k >= 0 && k < image.cols && l >= 0 && l < image.rows) {
						...
						double d = ComputeDist(j, Point(k,l), colour);
...
* Output: -
*/
void Slic::CreateConnectivity(Mat image) {
...
*
* Input : The image to display upon (Mat*) and the colour (CvScalar).
* Output: -
*/
void Slic::DisplayCenterGrid(Mat image, Scalar colour) {
	for (int i = 0; i < (int) centers.size(); i++) {
		cv::circle(image, Point(centers[i][3], centers[i][4]), 2, colour, 2);
	}
}
...
*
* Input : The target image (Mat*) and contour colour (CvScalar).
* Output: -
*/
void Slic::DisplayContours(Mat image, Vec3b colour) {
	...
	* is already taken to be a contour. */
	...
	for (int i = 0; i < (int)contours.size(); i++) {
		...
		//image.at<Scalar>(contours[i].y,contours[i].x,colour);
	}
}
...
* Output: -
*/
void Slic::ColourWithClusterMeans(Mat image) {
	vector<Vec3b> colours(centers.size());
